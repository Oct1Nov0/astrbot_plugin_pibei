...
@register("pibei", "Oct1Nov0", "谷圈云黑库", "1.1.0", "https://github.com/Oct1Nov0/astrbot_plugin_pibei")
class PibeiPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        groups = self._get_whitelist()
        logger.info(f"[云黑库] 插件已加载，管理员{len(admins)}人，白名单群{len(groups)}个，数据库：{self.db_path}")

    def _get_admins(self):
        raw = self.config.get("admin_qq_list", "")
        if not raw:
            return []
    ...
    async def add_case(self, event: AstrMessageEvent):
        """录入云黑记录。格式：/录入 QQ号 分类 金额 事件经过"""
        if not self._check_whitelist(event):
            return
        if not self._is_admin(event):
            yield event.plain_result("只有管理员才能录入云黑记录。")
            return
        ...
        if not c.fetchone():
            ...
            c2.execute("SELECT name FROM categories WHERE enabled=1 ORDER BY id")
            cats = "、".join(r["name"] for r in c2.fetchall())
            c2.connection.close()
            yield event.plain_result(
    ...
    async def query_case(self, event: AstrMessageEvent):
        ...
        conn.close()
        if not rows:
            yield event.plain_result(f"未查询到 QQ {qq} 的有效云黑记录。")
            return
        lines = ["⚠️ 风险记录", f"QQ：{qq}", f"有效记录：{len(rows)}条", ""]
        for r in rows:
